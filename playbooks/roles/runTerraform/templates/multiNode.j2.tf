...
resource "libvirt_volume" "os_image_ubuntu" {
  ...
  pool   = "default"
  source = "{{ playbook_dir }}/roles/downloadNodeIso/files/ubuntu-18.04-server-cloudimg-amd64.img"
}
...
users:
  - name: root
    ssh-authorized-keys:
      - ${file("{{ lookup('env','HOME') }}/.ssh/id_rsa.pub")}
growpart:
  mode: auto
  devices: ['/']
EOF
...
output "ip_master" {
  value = "${libvirt_domain.domain_ubuntu_k8s_master.network_interface.0.addresses.0}"
}

output "ip_worker1" {
  value = "${libvirt_domain.domain_ubuntu_k8s_worker1.network_interface.0.addresses.0}"
}

output "ip_worker2" {
  value = "${libvirt_domain.domain_ubuntu_k8s_worker2.network_interface.0.addresses.0}"
}
